...
function init() {
    ...
    //POST PROCESSING
		...
		renderPass = new THREE.RenderPass( sceneTV, camera );
...
function initCamera() {
    ...
    camera.lookAt(sceneTV.position);
}
...
function initMesh() {
  ...
  mtlLoader.load('../models/OldTV3.mtl', function (materials) {
      ...
      loader.load('../models/OldTV3.obj', function(geometry, materials) {
          ...
          mesh.children[1].material = normalmat;
          initVideo(mesh);
          sceneTV.add(mesh);
      });
  });
...
//  }
  ...
  parameters = { color: 0xffffff, map: videoTexture };
...
function render() {
    ...
    //renderer.render(sceneTV, camera);
}
})();
